    ...
    root.innerHTML = `
      <div class="rounded-3xl border border-yellow-200 bg-yellow-50 p-5 text-right">
        ...
        <div class="rounded-2xl bg-white/70 border border-yellow-200 p-4 text-xs leading-6" dir="ltr">
          ...
          <p><strong>Publishable key:</strong> ${config.publishableKey ? 'Saved in this browser' : 'Missing — save it from Advanced setup in #/admin/login first'}</p>
        </div>
        <a href="#/admin/login" class="mt-4 inline-flex rounded-xl bg-terracotta px-5 py-3 text-sm font-extrabold text-white hover:bg-red-700">الرجوع إلى بوابة الأدمن</a>
      </div>
    `;
  }
  ...
  function renderForm(root) {
    ...
    root.innerHTML = `
      <form data-admin-reset-password-form class="rounded-3xl border border-gray-200 bg-gray-50 p-5 text-right">
        ...
        <div class="mt-5 flex flex-col sm:flex-row gap-3 justify-start">
          ...
          <a href="#/admin/login" class="rounded-xl bg-white border border-gray-200 px-6 py-3 text-sm font-extrabold text-gray-700 hover:bg-gray-50 text-center">إلغاء والرجوع</a>
        </div>
      </form>
    `;
    ...
    form?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        setStatus(root, 'تم تغيير كلمة السر بنجاح. ارجع الآن إلى بوابة الأدمن وسجل الدخول بكلمة السر الجديدة.', 'success');
