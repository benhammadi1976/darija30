(function () {
  ...
  function imInMoroccoMarkup(phrase) {
    ...
    return `
      <section class="im-morocco-card mt-6" data-im-morocco data-phrase-id="${escapeHtml(phrase?.id || '')}">
        <div class="im-morocco-card__copy">
          ...
          <p class="im-morocco-card__text">Now enter the taxi scene with your voice.</p>
        </div>
        ...
        <p class="im-morocco-card__status" data-im-morocco-status>Record your voice above, then play the Moroccan scene with your voice.</p>
      </section>
    `;
  }
  ...
  function bindNowYouSpeakRecorders(root) {
    root.querySelectorAll('[data-now-you-speak]').forEach((card) => {
      ...
      const setButtonState = (state) => {
        ...
        primaryButton.classList.remove('is-recording', 'is-ready', 'is-playing');

        if (state === 'recording') {
          primaryButton.textContent = '■ Stop recording';
      ...
      const startRecording = async () => {
        try {
          ...
          setStatus('Recording... say the phrase now. Tap again to stop.', 'is-recording');
        } catch (error) {
          cleanupStream();
          setButtonState('idle');
      ...
      audio?.addEventListener('ended', () => {
        setButtonState('idle');
        setStatus('Want another try? Tap the button and say it again.', 'is-success');
      });
      ...
      const setReadyState = () => {
        ...
        if (status) {
          status.textContent = hasRecording
            ...
            : 'Record your voice above, then play the Moroccan scene with your voice.';
        }
      };
