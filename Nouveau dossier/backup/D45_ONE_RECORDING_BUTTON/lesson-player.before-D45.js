(function () {
      ...
      <div class="mb-5 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-left">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div class="flex flex-wrap gap-2" dir="ltr">
            <a href="#/admin/lessons" class="bg-white border border-blue-200 text-blue-700 px-3 py-2 rounded-xl text-xs font-black hover:bg-blue-100">Admin lessons</a>
  ...
  function sceneVisualMarkup(phrase) {
    ...
    return `
      <div class="memory-visual-card mt-5 text-left" data-visual-system="scene-image-plus-ui-overlay">
        ...
        <p class="memory-visual-caption">A quick visual memory cue for this phrase. The video shows the full dialogue.</p>
      </div>
    `;
  ...
  function nowYouSpeakMarkup(phrase) {
    ...
    return `
      <div class="now-you-speak-card mt-5" data-now-you-speak data-phrase-id="${escapeHtml(phrase?.id || '')}">
        ...
        <p class="now-you-speak-card__prompt">After the real scene, record yourself saying:</p>
        <p class="now-you-speak-card__phrase">${escapeHtml(phraseText)}</p>
        <div class="now-you-speak-card__controls">
          <button type="button" class="now-you-speak-btn now-you-speak-btn--record" data-record-start>● Record</button>
          <button type="button" class="now-you-speak-btn" data-record-stop disabled>■ Stop</button>
          <button type="button" class="now-you-speak-btn" data-record-play disabled>▶ Play My Voice</button>
          <button type="button" class="now-you-speak-btn now-you-speak-btn--ghost" data-record-reset disabled>↻ Try Again</button>
        </div>
        <audio class="now-you-speak-card__audio hidden" data-record-audio controls></audio>
  ...
  function bonusPhrasesMarkup(lesson, phraseIndex) {
    const bonus = Array.isArray(lesson?.bonusPhrases) ? lesson.bonusPhrases : [];
    const total = lesson?.phrases?.length || 0;
    if (!bonus.length || phraseIndex < total - 1) return '';
    return `
      <div class="mt-8 rounded-2xl border border-green-100 bg-green-50 p-5">
        <p class="text-xs font-extrabold uppercase tracking-wide text-green-700 mb-2">Bonus Phrases</p>
        <h3 class="text-lg font-extrabold text-medina mb-2">Extra confidence after the 5 core phrases</h3>
        <p class="text-sm text-gray-600 mb-4">These are optional. The lesson is complete with the five core phrases, but these extras help you sound more natural.</p>
        <div class="grid sm:grid-cols-3 gap-3">
          ${bonus.map((item) => `
            <div class="rounded-xl bg-white border border-green-100 p-4">
              <p class="font-mono font-extrabold text-green-700">${escapeHtml(item.friendlyLatin)}</p>
              <p class="text-sm text-gray-600 mt-1">${escapeHtml(item.meaning)}</p>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <div class="lesson-player-shell bg-white rounded-3xl shadow-xl p-6 md:p-8 border border-gray-100">
        ...
        <div class="lesson-main-grid grid lg:grid-cols-[0.95fr_1.25fr] gap-6 items-stretch">
          <div class="rounded-2xl bg-gray-50 border border-gray-100 p-5 text-left">
            ...
            ${methodPill()}
            ${phrase.sceneVideo ? `
              <div class="mt-5 overflow-hidden rounded-2xl border border-gray-200 bg-black shadow-sm">
                <video class="w-full aspect-video object-cover" controls playsinline preload="metadata" ${phrase.scenePoster ? `poster="${escapeHtml(phrase.scenePoster)}"` : ''}>
                  <source src="${escapeHtml(phrase.sceneVideo)}" type="video/mp4">
                  Your browser does not support this video.
                </video>
              </div>
              <p class="mt-2 text-xs font-bold text-gray-500">🎬 ${escapeHtml(phrase.sceneCaption || 'Watch the situation before repeating the phrase.')}</p>
              ${dialogueLinesMarkup(phrase)}
            ` : ''}
            ${phrase.moroccanChat ? `
          ...
          <div class="rounded-2xl border border-gray-100 p-6 text-center">
            ...
            <p class="audio-recording-hint inline-block rounded-full px-4 py-2 text-xs font-bold mb-6">If audio is not recorded yet, the button will show the exact missing MP3 path.</p>
        ...
        ${phrase.cultureNote ? `
          <div class="mt-6 bg-yellow-50 border border-yellow-100 rounded-2xl p-5 text-left">
            <p class="font-bold text-yellow-800 mb-1">Culture Note</p>
            <p class="text-yellow-900 text-sm">${escapeHtml(phrase.cultureNote)}</p>
          </div>
        ` : ''}

        ${phrase.memoryHook ? `
          <div class="mt-4 bg-green-50 border border-green-100 rounded-2xl p-5 text-left">
            <p class="font-bold text-green-800 mb-1">Memory Hook</p>
            <p class="text-green-900 text-sm">${escapeHtml(phrase.memoryHook)}</p>
          </div>
        ` : ''}
        ...
        ${!compact ? `
          ...
          ${quiz.length ? renderQuizMarkup(quiz, lesson, phrase) : ''}
  ...
  function bindNowYouSpeakRecorders(root) {
    root.querySelectorAll('[data-now-you-speak]').forEach((card) => {
      const startButton = card.querySelector('[data-record-start]');
      const stopButton = card.querySelector('[data-record-stop]');
      const playButton = card.querySelector('[data-record-play]');
      const resetButton = card.querySelector('[data-record-reset]');
      ...
      startButton?.addEventListener('click', async () => {
        try {
          ...
          mediaRecorder.addEventListener('stop', () => {
            ...
            card.classList.remove('is-recording');
            setStatus('Recording ready. Play your voice and compare with the native phrase.', 'is-success');
          });
          ...
          if (resetButton) resetButton.disabled = true;
          card.classList.add('is-recording');
          setStatus('Recording... say the phrase now.', 'is-recording');
        } catch (error) {
          cleanupStream();
          if (startButton) startButton.disabled = false;
          if (stopButton) stopButton.disabled = true;
